...
class Evaluator(object):
  ...
  def __init__(self, report_file='report.txt', height = 960, width = 720):
    self.height, self.width = height, width
    # = parser.ObjectronParser(self.height, self.width)
    ...
    rot_adjustment = np.array([
                              ...
                              [1, 0, 0], 
                              [0, 0, -1]])
    self.trafo_2_obj = np.eye(4)
    self.trafo_2_obj[:3, :3] = rot_adjustment
  ...
  # 
  # 
  def convert(self, result, coco_label, intrinsics): # instead of predict
    """
    ...
    for box_tensor in result['boxes_3d'].tensor: # box3d_camera if convert_valid_boxes called before

      box_tensor = box_tensor.numpy() 
      # rotation, translation, scale
      box = Box.Box.from_transformation(box_tensor[6:], box_tensor[:3], box_tensor[3:6]) 
      # project 9 keypoints to 2D
      vertices_2d = points_cam2img(tensor(box.vertices).float(), intrinsics)
      vertices_2d[:, 0] /= self.width
      vertices_2d[:, 1] /= self.height

      # transform box back to objectron convention
      obj_box = box.apply_transformation(self.trafo_2_obj)
    ...
    for box_tensor in coco_label['gt_bboxes_3d'].tensor:
  ...
  # official code snippet to calculate visbility manually
  # https://github.com/google-research-datasets/Objectron/issues/37
  def check_object_visibility(self, keypoints) -> float:
      ...
      if any(not (-0.5 < x < 1.5 and -0.5 < y < 1.5) for x, y, in keypoints[1:]):
        visibility = 0.0

      return visibility


  def evaluate_single(self, results_raw, gt_labels, img_info): # TODO: loop over batch / all images moved to dataset
    """Evaluates boxes of a single sample."""
    ...
    intrinsics = img_info['cam_intrinsic']
    intrinsics = from_numpy(np.array(intrinsics)).float()
    #pred = self.model.predict(np.asarray(images), batch_size=len(batch))
    results, labels = self.convert(results_raw, gt_labels, intrinsics)

    #for boxes, gt_boxes in zip(results, labels):  # for every image / sample    
    ...
    instances, instances_3d = zip(*labels)

    instances = np.array(instances) # match function needs an array and not a list of arrays
    ...
    # instances: A ?*9*2 array of annotated instances. Each instance is a 9*2 array.
    # visibilities: An array of the visibilities of the instances.

    for box in results: # for all predicted boxes, check find the best gt box
      box_point_2d, box_point_3d = box # predicted results
  ...
  # def evaluate(self, results_raw, gt_labels, img_info): # TODO: all or for each image?
  #   """Evaluates boxes of a list of samples."""
  #   # images, labels, projs, planes = [], [], [], []
  #   # for serialized in batch:
  #   #   example = tf.train.Example.FromString(serialized)
  #   #   image, label = self.encoder.parse_example(example)
  ...
  #   for boxes, label in zip(results, labels):  # for every image / sample    
  #     instances = label['2d_instance']
  #     instances_3d = label['3d_instance']
  #     visibilities = label['visibility']
  #     num_instances = 0
  ...
  #     for box in boxes: # for all predicted boxes, check if we find the current selected gt box
  #       box_point_2d, box_point_3d = box # predicted results
  #       index = self.match_box(box_point_2d, instances, visibilities)
  #       if index >= 0:
  #         num_matched += 1
  ...
  def write_report(self, print_cli=True):
    ...
    with open(self.report_file, 'w+') as f:
      ...
      print('Number Matched: ', self._matched)
      # print whole summary after writing to file
      if print_cli:
        f.flush()
        f.seek(0)
        print(f.read())
  ...
  def get_result_dict(self):
    ...
    result_dict = {
        'Mean_Error_2D': safe_divide(self._error_2d, self._matched),
        'Mean_3D_IoU': safe_divide(self._iou_3d, self._matched),
