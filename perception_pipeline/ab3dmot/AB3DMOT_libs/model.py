...
from .bbox_utils import convert_3dbox_to_8corner, iou3d, evaluate_iou3d
...
def associate_detections_to_trackers(detections, trackers, iou_threshold=0.01):   
	...
	for d, det in enumerate(detections):
		for t, trk in enumerate(trackers):
			...
			iou_matrix[d, t] = evaluate_iou3d(det, trk) #[0]
	# matched_indices = linear_assignment(-iou_matrix)      # houngarian algorithm, compatible to linear_assignment in sklearn.utils

	# print("----------IOU------------: ", iou_matrix)
	row_ind, col_ind = linear_sum_assignment(-iou_matrix)      # houngarian algorithm
...
class AB3DMOT(object):			  # A baseline of 3D multi-object tracking
	...
	def update(self, dets_all):
		...
		Params:
		  dets_all: dict
			dets - a numpy array of detections in the format [[h,w,l,x,y,z,theta],...] -> [x,y,z,h,w,l,alpha,beta,gamma],...]
		...
		"""
		dets, info = dets_all['dets'], dets_all['info']         # dets: N x 9, float numpy array
		...
		trks = np.zeros((len(self.trackers), 9))         # N x 7 , # get predicted locations from existing trackers.
		to_del = []
		ret = []
		for t, trk in enumerate(trks):
			pos = self.trackers[t].predict().reshape((-1, 1))
			trk[:] = [pos[0], pos[1], pos[2], pos[3], pos[4], pos[5], pos[6], pos[7], pos[8]]       
		...
		if dets.size > 0:
			...
			dets_8corner = []
		#print("dets8corners: ", dets_8corner)
		...
		#print("trks_8corner: ", trks_8corner)
